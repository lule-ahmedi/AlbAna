...
CNN_LSTM.add(layers.Embedding(len(word_index) + 1,
                            EMBEDDING_DIM,
...
CNN_LSTM.add(Embedding(len(word_index), EMBEDDING_DIM, input_length=MAX_SEQUENCE_LENGTH))
'''

CNN_LSTM.add(SpatialDropout1D(0.3))

CNN_LSTM.add(Conv1D(512, 3, activation='relu'))

CNN_LSTM.add(MaxPooling1D())

CNN_LSTM.add(Bidirectional(LSTM(32, return_sequences=True)))

#CNN_LSTM.add(SeqSelfAttention(attention_width=8, attention_activation='sigmoid', name='Attention',))

CNN_LSTM.add(Flatten())

CNN_LSTM.add(Dense(64, activation='relu'))

CNN_LSTM.add(Dense(3, activation='softmax'))
...
#==============================================================================
CNN_LSTMHistory=CNN_LSTM.fit(x_train, y_train, epochs = 2, batch_size = 32,verbose=2, validation_data=(x_val,y_val), shuffle=True)

plot_history(CNN_LSTMHistory)

full_multiclass_report(CNN_LSTM, x_val, y_val, encoder.inverse_transform(np.arange(3)))
